...
class MyApp extends LitElement {
  ...
  public toast: null | LitToast = null;
  ...
  @state()
  private _padding = "16px";

  @state()
  private _bottom = "40px";

  @state()
  private _zIndex = "2";

  @state()
  private _fontSize = "1em";

  @state()
  private _borderRadius = "2px";
  ...
  @state()
  private _backgroundColor = "#292929";
  ...
  protected firstUpdated(): void {
    ...
    this.toast = this.shadowRoot?.querySelector<LitToast>("lit-toast") ?? null;

    // If 'path' or 'composedPath()' unavailable, don't show options
    this.addEventListener("click", (event: Event) => { this._showOptions = event.composed; });

    this.click();
  }

  private _callToast(): void {
    (this.toast as LitToast).show(this._message).then(() => {
      console.log('Toast finished');
    });
  }

  private _updateMessage(event: Event): void {
    const path = event.composedPath();
    if (path.length > 0 && Object.prototype.hasOwnProperty.call(path[0], "value")) {
      this._message = (path[0] as ITargetPath).value;
    }
  }

  private _updatePadding(event: Event): void {
    const path = event.composedPath();
    if (path.length > 0 && Object.prototype.hasOwnProperty.call(path[0], "value")) {
      this._padding = (path[0] as ITargetPath).value;
      (this.toast as LitToast).style.setProperty('--lt-padding', this._padding);
    }
  }

  private _updateBottom(event: Event): void {
    const path = event.composedPath();
    if (path.length > 0 && Object.prototype.hasOwnProperty.call(path[0], "value")) {
      this._bottom = (path[0] as ITargetPath).value;
      (this.toast as LitToast).style.setProperty('--lt-bottom', this._bottom);
    }
  }

  private _updateZIndex(event: Event): void {
    const path = event.composedPath();
    if (path.length > 0 && Object.prototype.hasOwnProperty.call(path[0], "value")) {
      this._zIndex = (path[0] as ITargetPath).value;
      (this.toast as LitToast).style.setProperty('--lt-z-index', this._zIndex);
    }
  }

  private _updateFontSize(event: Event): void {
    const path = event.composedPath();
    if (path.length > 0 && Object.prototype.hasOwnProperty.call(path[0], "value")) {
      this._fontSize = (path[0] as ITargetPath).value;
      (this.toast as LitToast).style.setProperty('--lt-font-size', this._fontSize);
    }
  }

  private _updateBorderRadius(event: Event): void {
    const path = event.composedPath();
    if (path.length > 0 && Object.prototype.hasOwnProperty.call(path[0], "value")) {
      this._borderRadius = (path[0] as ITargetPath).value;
      (this.toast as LitToast).style.setProperty('--lt-border-radius', this._borderRadius);
    }
  }

  private _updateBorder(event: Event): void {
    const path = event.composedPath();
    if (path.length > 0 && Object.prototype.hasOwnProperty.call(path[0], "value")) {
      this._border = (path[0] as ITargetPath).value;
      (this.toast as LitToast).style.setProperty('--lt-border', this._border);
    }
  }

  private _updateColor(event: Event): void {
    const path = event.composedPath();
    if (path.length > 0 && Object.prototype.hasOwnProperty.call(path[0], "value")) {
      this._color = (path[0] as ITargetPath).value;
      (this.toast as LitToast).style.setProperty('--lt-color', this._color);
    }
  }

  private _updateBackgroundColor(event: Event): void {
    const path = event.composedPath();
    if (path.length > 0 && Object.prototype.hasOwnProperty.call(path[0], "value")) {
      this._backgroundColor = (path[0] as ITargetPath).value;
      (this.toast as LitToast).style.setProperty(
        '--lt-background-color',
  ...
  private _updateFontFamily(event: Event): void {
    const path = event.composedPath();
    if (path.length > 0 && Object.prototype.hasOwnProperty.call(path[0], "value")) {
      this._fontFamily = (path[0] as ITargetPath).value;
      (this.toast as LitToast).style.setProperty('--lt-font-family', this._fontFamily);
    }
  }
}
